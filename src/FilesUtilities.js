...
var indexDirectory = path.join(dataDir, 'EventIndex.json')
var torneosDir = path.join(dataDir, 'Torneos')
...
function SaveTorneo(jsonData) {
    ...
    var filePath = path.join(torneosDir, jsonData.name + '.json')
...
function GetIndexData() {
    ...
    if (fs.existsSync(indexDirectory)) {
        var indexString = fs.readFileSync(indexDirectory).toString()
...
function ReadJSON(path) {
    var stringData = fs.readFileSync(path).toString()
...
function GetPenalidades() {
    var filePath = path.resolve(dataDir, 'penalidades.json')
...
function GetTorneo(name) {
    var json = ReadJSON(path.join(torneosDir, name + '.json'))
    return json
}
